...
new class extends Component {
    ...
    public $winnerIds = []; // Array to store winner for each challenge
    ...
    public function loadChallenges()
    {
        try {
            ...
            $this->challenges = Challenge::where('witness_id', $userId)
                ...
                ->with(['challenger', 'opponent', 'team']) // Added team relationship
                ->orderBy('created_at', 'desc')
                ->get();

            // Initialize winner selection array
            $this->winnerIds = [];
    ...
    public function submitOutcome($challengeId)
    {
        ...
        try {
            ...
            // Get challenge with all necessary relationships
            ...
            DB::transaction(function () use ($challenge, $winnerId) {
                ...
                if (!$winner || !$loser) {
                    throw new \Exception('Winner or loser not found');
                }

                // Get current ranks from team pivot
                $winnerTeamUser = $winner->teams()->where('team_id', $challenge->team_id)->first();
                $loserTeamUser = $loser->teams()->where('team_id', $challenge->team_id)->first();

                if (!$winnerTeamUser || !$loserTeamUser) {
                    throw new \Exception('Users not found in team');
                }

                $winnerCurrentRank = $winnerTeamUser->pivot->rank;
                $loserCurrentRank = $loserTeamUser->pivot->rank;

                // Only swap ranks if winner has lower rank (higher number)
                $ranksSwapped = false;
                ...
                $this->sendToDiscord($winner, $loser, $challenge, $ranksSwapped);
            });
    ...
    protected function sendToDiscord($winner, $loser, $challenge, $ranksSwapped)
    {
        try {
            $discord = app(\App\Services\DiscordService::class);
            
            // Add winner/loser information to the challenge temporarily for the notification
            ...
            // Send updated rankings if ranks were swapped
    ...
    private function swapRanks(User $winner, User $loser, Challenge $challenge)
    {
        ...
        $winnerPreviousRank = $winnerPivot->rank;
        $loserPreviousRank = $loserPivot->rank;

        // Swap the ranks in the pivot table
        ...
        Log::info('Ranks swapped successfully', [
            'challenge_id' => $challenge->id,
            'winner_id' => $winner->id,
            'winner_rank_change' => $winnerPreviousRank . ' -> ' . $loserPreviousRank,
            'loser_id' => $loser->id,
            'loser_rank_change' => $loserPreviousRank . ' -> ' . $winnerPreviousRank,
        ]);
    }
    ...
    public function getWinnerForChallenge($challengeId)
    {
        return $this->winnerIds[$challengeId] ?? null;
    }

    public function setWinnerForChallenge($challengeId, $winnerId)
    {
        $this->winnerIds[$challengeId] = $winnerId;
    }
}; ?>
...
" class="container mt-1 bg-white rounded-lg shadow dark:bg-gray-800">
    ...
    <div class="p-5">
            ...
            @else
                <div class="space-y-4">
                    @foreach ($challenges as $challenge)
                        <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border-l-4 border-blue-500 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-150">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center space-x-2 mb-4">
                                        ...
                                        <span class="text-gray-500 dark:text-gray-400 font-bold">vs</span>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        @if(!empty($winnerIds[$challenge->id]))
                                            ...
                                            @if($winnerTeamUser && $loserTeamUser && $winnerTeamUser->pivot->rank > $loserTeamUser->pivot->rank)
                                                <div class="p-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded text-sm">
        ...
        <!-- Loading Overlay - Only show when there are challenges -->
        @if (!empty($challenges) && $challenges->isNotEmpty())
            <div wire:loading.flex wire:target="submitOutcome" 
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-gray-700 dark:text-gray-300">Submitting outcome...</span>
                    </div>
                </div>
            </div>
        @endif
